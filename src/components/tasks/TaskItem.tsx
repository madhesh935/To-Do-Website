...
function TaskEditor({ task, onCancel, onSave }: { task: Task; onCancel: () => void; onSave: Props['onSave'] }) {
  ...
  return (
    <form className="task-editor" onSubmit={submit} onKeyDown={(event) => {
      ...
      <div className="editor-actions">
        <button className="button primary" type="submit"><Icon name="check" />Save Changes</button>
        <button className="button secondary" type="button" onClick={onCancel}>Cancel</button>
        <span className="keyboard-hint">Esc to cancel</span>
      </div>
    </form>
  );
}
...
export const TaskItem = memo(function TaskItem({ task, editing, onEdit, onSave, onToggle, onDelete }: Props) {
  ...
  return (
    <li
      className={`task-item ${styles.priorityRail}${task.completed ? ' is-completed' : ''}${editing ? ' is-editing' : ''}`}
      data-task-id={task.id}
      data-priority={task.priority.toLowerCase()}
    >
      {editing ? (
        <TaskEditor task={task} onCancel={cancel} onSave={onSave} />
      ) : (
        <>
          <label className="task-check">
            ...
            />
            <span className="checkbox-visual"><Icon name="check" /></span>
          </label>
          ...
          <div className="task-actions">
            <button ref={editRef} type="button" className="icon-button" onClick={() => onEdit(task.id)} aria-label={`Edit ${task.title}`} title="Edit task">
              <Icon name="edit" />
            </button>
            <button type="button" className="icon-button delete-button" onClick={() => onDelete(task.id)} aria-label={`Delete ${task.title}`} title="Delete task">
              <Icon name="trash" />
            </button>
          </div>
        </>
